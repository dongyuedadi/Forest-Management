...
out_df['林分蓄积']=out_df.groupby('plot')['树种蓄积'].transform('sum')
out_df['树种蓄积占比']=round(out_df['树种蓄积']/out_df['林分蓄积'],10)
S=len(out_df['树种'].unique())
out_df['树种组成指数'] = out_df.groupby(['plot'])['树种蓄积占比'].transform(lambda x:( -np.sum(x * np.log(x)))/np.log(S))
out_df.to_excel(os.path.join(out_path,'倒木林分蓄积—树种组成.xlsx'),index=False)
SHANNON_VOL=out_df.drop_duplicates('plot')
SHANNON_VOL.to_excel(os.path.join(out_path,'倒木林分蓄积—树种组成汇总.xlsx'),index=False)
print(SHANNON_VOL)
