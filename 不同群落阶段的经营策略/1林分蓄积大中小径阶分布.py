...
result = pd.DataFrame(columns=['Plot', '小径木蓄积', '中径木蓄积', '大径木蓄积'])
# 遍历每个Plot，计算大中小径木的蓄积
...
for Plot in Plots:
    ...
    small_diameter_volume = Plot_data[Plot_data['分类'] == '小径木']['径阶蓄积'].sum()
    medium_diameter_volume = Plot_data[Plot_data['分类'] == '中径木']['径阶蓄积'].sum()
    large_diameter_volume = Plot_data[Plot_data['分类'] == '大径木']['径阶蓄积'].sum()
...
print(result)
result.to_excel(os.path.join(out_path,'林分大中小径木蓄积.xlsx'),index=False)
