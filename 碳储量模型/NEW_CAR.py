...
def residuals_with_regularization(params, X, y1, y2):
    ...
    regularization_term = lambda_reg * (((a0_1 - a0_2) ** 2 + (a0_2 - a0_3) ** 2 + (a0_3 - a0_1) ** 2)+
                                        ((k0_1 - k0_2) ** 2 + (k0_2 - k0_3) ** 2 + (k0_3 - k0_1) ** 2))
    # 返回总残差（原始残差 + 正则化项）
    return np.concatenate((original_residuals, [regularization_term]))

# 设置参数边界
bounds_lower = [10,10, 10, 0, 0, 0, 0, 0, 0, 0, 0]
bounds_upper = [50, 50, 50, 100, 100, 100, 100, 100, 100, 100, 100]
...
with pd.ExcelWriter(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林碳储量速查表.xlsx',
    ...
    for s in dummies_list:
        ...
        scia = scia_list[s - 1]
        scib = scib_list[s - 1]
        CAR_matrix = np.zeros((len(AGE[s-1]), len(SCI_list)))
        ...
        df_CAR_matrix = pd.DataFrame(CAR_matrix, index=AGE[s-1], columns=SCI_list)
        sheet_name = f'年龄{s}'
        df_CAR_matrix.to_excel(writer, sheet_name=sheet_name, index=True)

df1 = pd.read_excel(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林碳储量速查表.xlsx',
                            sheet_name='年龄1')
df2 = pd.read_excel(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林碳储量速查表.xlsx',
                            sheet_name='年龄2')
df3 = pd.read_excel(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林碳储量速查表.xlsx',
                            sheet_name='年龄3')
combined_df = pd.concat([df1, df2, df3], axis=0)
combined_df.to_excel(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\new\小兴安岭珍贵硬阔叶林碳储量速查表1.xlsx', index=False)
combined_df.set_index('Unnamed: 0', inplace=True)
for i in SCI_list:
    N_AGE = combined_df.index.values
    N_CAR = combined_df.loc[:, i].values
    plt.plot(N_AGE, N_CAR, label=f'SCI={i}')
