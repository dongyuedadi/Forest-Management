...
dummies_list=[1,2,3]
Age1=list(range(5,21,5))
Age2=list(range(25,41,5))
Age3=list(range(45,101,5))
AGE=[Age1,Age2,Age3]
...
with pd.ExcelWriter(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林树高速查表.xlsx',
    ...
    for s in dummies_list:
        a = a1[s - 1]
        b = b1[s - 1]
        Height_matrix = np.zeros((len(AGE[s-1]), len(SCI_list)))
        ...
        df_Height_matrix = pd.DataFrame(Height_matrix, index=AGE[s-1], columns=SCI_list)
        sheet_name = f'年龄{s}'
        df_Height_matrix.to_excel(writer, sheet_name=sheet_name, index=True)

df1 = pd.read_excel(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林树高速查表.xlsx', sheet_name='年龄1')
df2 = pd.read_excel(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林树高速查表.xlsx', sheet_name='年龄2')
df3 = pd.read_excel(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林树高速查表.xlsx', sheet_name='年龄3')
combined_df = pd.concat([df1, df2, df3], axis=0)
combined_df.to_excel(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\new\小兴安岭珍贵硬阔叶林树高速查表1.xlsx', index=False)
combined_df.set_index('Unnamed: 0', inplace=True)
for i in SCI_list:
    H_AGE=combined_df.index.values
    Height =combined_df.loc[:, i].values
    plt.plot(H_AGE, Height, label=f'SCI={i}')
