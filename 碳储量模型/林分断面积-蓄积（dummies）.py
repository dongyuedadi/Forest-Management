...
with pd.ExcelWriter(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林胸高断面积速查表.xlsx',
    ...
    for s in dummies_list:
        a0 = a0_list[s-1]
        k0 = k0_list[s-1]
        BAS_matrix = np.zeros((len(Age), len(SCI_list)))
        for idx_age, val_age in enumerate(Age):
            for idx_i, val_i in enumerate(SCI_list):
                BAS_matrix[idx_age, idx_i] = a0 * (val_i ** a1)*(1 - np.exp((-k0) * ((SDI_out/ 10000) ** k1) * val_age)) ** c
        #print(f'\n哑变量{s}:小兴安岭珍贵硬阔叶林胸高断面积速查表\n{BAS_matrix}')
        df_BAS_matrix = pd.DataFrame(BAS_matrix, index=Age, columns=SCI_list)
        sheet_name = f'哑变量{s}'
        df_BAS_matrix.to_excel(writer, sheet_name=sheet_name, index=True)
        for i in SCI_list:
            BAS = a0 * (i ** a1)*(1 - np.exp((-k0) * ((SDI_out/ 10000) ** k1) * Age)) ** c
            plt.plot(Age, BAS, label=f'SCI={i}')
        ...
        filename = f'BAS哑变量_{s}_curve.png'
...
with pd.ExcelWriter(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林蓄积量速查表.xlsx',
    ...
    for s in dummies_list:
        ...
        scia = scia_list[s - 1]
        scib = scib_list[s - 1]
        Volumes_matrix = np.zeros((len(Age), len(SCI_list)))
        for idx_age, val_age in enumerate(Age):
            for idx_i, val_i in enumerate(SCI_list):
                Height = val_i * (1 / (((scia * (1 - np.exp(-scib * 50)) ** scic) / (scia * (1 - np.exp(-scib * val_age)) ** scic))))
        ...
        df_Volumes_matrix = pd.DataFrame(Volumes_matrix, index=Age, columns=SCI_list)
        sheet_name = f'哑变量{s}'
        df_Volumes_matrix.to_excel(writer, sheet_name=sheet_name, index=True)
        for i in SCI_list:
            Height = i * (1 / (((scia * (1 - np.exp(-scib * 50)) ** scic) / (scia * (1 - np.exp(-scib * Age)) ** scic))))
            BAS = a0 * (i ** a1)*(1 - np.exp((-k0) * ((SDI_out/ 10000) ** k1) * Age)) ** c
