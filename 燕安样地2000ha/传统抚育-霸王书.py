...
file_path=r'C:\Users\hys5637428\Desktop\不同阶段经营模式\燕安大径材培育规划\小班'
...
for i in name_list:
    ...
    # 选择目的树种中的优质木
    selected_trees = filtered_sorted_trees
    selected_trees.dropna(inplace=True)
    # 将选定的树木转换回DataFrame
    ...
    # 将干扰树转换回DataFrame和输出文件格式
    disruptions_trees_df = pd.DataFrame(disruptions_trees)
    disruptions_trees_df.dropna(inplace=True)
    print(disruptions_trees_df,basename,'\n')
    disruptions_trees_df.to_excel(os.path.join(out_path, f'{basename}干扰树.xlsx'), index=False)

    #绘图
    #树冠图
    print(disruptions_trees_df.columns,basename)
