...
params1 = [41, 41, 41, 0.24, 0.18, 0.18, 0.18, 2.89, 0.3]
params2 = [10.90, 6.42]
...
def objective_function(params, X, y1, y2):
    ...
    residual = np.concatenate((y1 - y_pred_1, y2 - y_pred_2))
    # 计算a0_1, a0_2, a0_3之间的差异的平方和作为惩罚项
    penalty = (params[0] - params[1]) ** 2 + (params[0] - params[2]) ** 2 + (params[1] - params[2]) ** 2
    # 返回残差的平方和与惩罚项的总和
    return np.sum(residual ** 2) + penalty
# 使用正确的格式定义参数边界
bounds = [
    (40, 60), (40, 60), (40, 60),  # 对应于 a0_1, a0_2, a0_3
    (0, 100), (0, 100), (0, 100), (0, 100), (0, 100), (0, 100),  # 对应于 a1, k0_1, k0_2, k0_3, k1, c
    (0, 100), (0, 100)  # 对应于 d0, d1
]
# 然后在调用 minimize 函数时使用这些边界
result = minimize(objective_function, initial_guess, args=(X, Y_BAS, y), bounds=bounds)
...
df_params.to_excel(os.path.join(out_path, '生物量参数值.xlsx'), index=False)
# 速查表矩阵设置
dummies_list = [1, 2, 3]
...
# 生物量矩阵
with pd.ExcelWriter(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林生物量速查表.xlsx',
    ...
    for s in dummies_list:
        ...
        scia = scia_list[s - 1]
        scib = scib_list[s - 1]
        BIO_matrix = np.zeros((len(AGE[s-1]), len(SCI_list)))
        for idx_age, val_age in enumerate(AGE[s-1]):
            for idx_i, val_i in enumerate(SCI_list):
                ...
                BIO_matrix[idx_age, idx_i] = BAS * Height * (d0 / (Height + d1))
        # print(f'\n哑变量{s}:小兴安岭珍贵硬阔叶林蓄积量速查表\n{BAS_matrix}')
        df_BIO_matrix = pd.DataFrame(BIO_matrix, index=AGE[s-1], columns=SCI_list)
        sheet_name = f'年龄{s}'
        df_BIO_matrix.to_excel(writer, sheet_name=sheet_name, index=True)

df1 = pd.read_excel(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林生物量速查表.xlsx',
                            sheet_name='年龄1')
df2 = pd.read_excel(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林生物量速查表.xlsx',
                            sheet_name='年龄2')
df3 = pd.read_excel(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\小兴安岭珍贵硬阔叶林生物量速查表.xlsx',
                            sheet_name='年龄3')
combined_df = pd.concat([df1, df2, df3], axis=0)
combined_df.to_excel(r'C:\Users\hys5637428\Desktop\帽儿山蘑菇调查\out\new\小兴安岭珍贵硬阔叶林生物量速查表1.xlsx', index=False)
combined_df.set_index('Unnamed: 0', inplace=True)
for i in SCI_list:
    N_AGE = combined_df.index.values
    N_BIO = combined_df.loc[:, i].values
    plt.plot(N_AGE, N_BIO, label=f'SCI={i}')
plt.rcParams['font.sans-serif'] = ['SimHei']  # 指定默认字体
plt.rcParams['axes.unicode_minus'] = False
plt.xticks(fontsize=15)
plt.yticks(fontsize=15)
plt.ylabel('生物量/$t$', fontsize=14)
plt.xlabel('年龄/years', fontsize=14)
plt.legend(fontsize=10)
# plt.title(f'生物量:哑变量{s}——Chapman_Richards', fontsize=20)
filename = f'生物量年龄_curve.png'
...
EvaluatingModelPerformance(y, best_predictions, len(best_params), sheetname='生物量')
